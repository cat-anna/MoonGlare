...
MainWindow::MainWindow(SharedModuleManager modmgr) : QMainWindow(nullptr), iModule(std::move(modmgr)) {
    ...
    connect(ui->buttonBox, &QDialogButtonBox::clicked, [this](QAbstractButton *item) { close(); });
    ...
    connect(ui->listWidget, &QListWidget::customContextMenuRequested, [this](const QPoint &point) {
        ...
        if (item) {
            ...
            menu.addSeparator();
            menu.addAction("Send", [this, item]() { Send(item->text().replace("|", "\n"), false); });
            menu.addSeparator();
            menu.addAction("Edit",
                           [this, item]() { ui->plainTextEdit->setPlainText(item->text().replace("|", "\n")); });
...
bool MainWindow::Initialize() {
    reconClient = GetModuleManager()->QueryModule<Tools::RemoteConsole::ReconClient>();

    auto appc = GetModuleManager()->QueryModule<RuntineModules::AppConfig>();
