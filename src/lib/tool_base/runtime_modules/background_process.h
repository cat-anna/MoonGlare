...
protected:
    ...
    SharedOutput output;
    std::string id;
    std::string readableName; // FIXME: possible race on GetXXX string functions!
    float progress = -1.0f;
    std::thread thread;
    bool canRun = false;
...
public:
    ...
    template <typename T, typename... ARGS> std::shared_ptr<T> CreateProcess(const std::string &id, ARGS... args) {
