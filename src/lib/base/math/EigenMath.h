...
template <class T, class S> T MathCast(const S &s) {
    static_assert(std::is_same<int, float>::value, "invalid cast!");
    throw false;
}

template <> inline fmat4 MathCast(const glm::mat4 &s) { return fmat4(&s[0][0]); }
template <> inline glm::fmat4 MathCast(const fmat4 &s) { return *reinterpret_cast<const glm::fmat4 *>(s.data()); }

template <> inline glm::fvec4 MathCast(const Quaternion &s) {
    return {
        s.x(),
        s.y(),
        s.z(),
...
template <> inline Quaternion MathCast(const glm::fvec4 &s) {
    return {
        s.w,
        s.x,
        s.y,
...
#ifdef BT_VECTOR3_H
template <> inline fvec3 MathCast(const btVector3 &s) {
    return {
        s.x(),
        s.y(),
        s.z(),
...
template <> inline Quaternion MathCast(const btQuaternion &s) {
    return {
        s.w(),
        s.x(),
        s.y(),
...
template <> inline btVector3 MathCast(const fvec3 &s) {
    return {
        s.x(),
        s.y(),
        s.z(),
...
template <> inline btQuaternion MathCast(const Quaternion &s) {
    return {
        s.w(),
        s.x(),
        s.y(),
...
#define GEN_2(SRC, DST)                                                                                                \
    template <> inline DST##2 MathCast(const SRC##2 & s) { return DST##2(s[0], s[1]); }
#define GEN_3(SRC, DST)                                                                                                \
    template <> inline DST##3 MathCast(const SRC##3 & s) { return DST##3(s[0], s[1], s[2]); }
#define GEN_4(SRC, DST)                                                                                                \
    template <> inline DST##4 MathCast(const SRC##4 & s) { return DST##4(s[0], s[1], s[2], s[3]); }
