...
void Settings::SaveToFile(const std::string &fileName) const {
    boost::filesystem::save_string_file(fileName, Serialize());
}
...
void Settings::LoadFromFile(const std::string &fileName) {
    try {
        ...
        std::string data;
        boost::filesystem::load_string_file(fileName, data);
        Deserialize(data);
    } catch (const std::exception &e) {
        AddLogf(Error, "Failed to load settings from file: %s", e.what());
    }
}
...
void Settings::Subscribe(std::shared_ptr<Settings::iChangeCallback> cb) { callbackList.emplace_back(cb); }
