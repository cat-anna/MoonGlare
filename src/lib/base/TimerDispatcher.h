...
template <typename TimerData, uint32_t TimerLimit_v> class TimerDispatcher {
  public:
    static constexpr uint32_t TimerLimit = TimerLimit_v;

    TimerDispatcher(InterfaceMap &ifaceMap)
        : handleTable(ifaceMap.GetInterface<HandleTable>()) {
        assert(handleTable);
