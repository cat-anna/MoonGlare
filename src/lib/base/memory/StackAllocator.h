...
template <template <class> class ALLOCATOR> struct StackAllocator {
    template <typename... ARGS> StackAllocator(ARGS... args) : m_Allocated(0), m_Storage(std::forward<ARGS>(args)...) {}
    ...
    size_t Reserve() const { return Capacity() - Allocated(); }
    size_t Capacity() const { return m_Storage.ElementCount(); }
    size_t Allocated() const { return m_Allocated; }
    void PartialClear(size_t alloc) { m_Allocated = alloc; }
    void Clear() { m_Allocated = 0; }
    ...
    template <typename T> T *Clone(const T *data, size_t Count) {
    ...
    template <typename T> T *Clone(const std::vector<T> &data) {
    ...
    template <typename T, size_t COUNT> T *Clone(const std::array<T, COUNT> &data) {
        T *ptr = Allocate<T>(data.size());
        if (ptr)
            memcpy(ptr, &data[0], data.size() * sizeof(T));
        return ptr;
    ...
    template <typename T, size_t COUNT> T *Clone(const T (&data)[COUNT]) {
        T *ptr = Allocate<T>(COUNT);
        if (ptr)
    ...
    template <class T> T *AllocateAligned(size_t Count = 1) {
        // static_assert (std::is_pod<T>::value, "Not a pod type!");
        if (Count == 0)
            return nullptr;
