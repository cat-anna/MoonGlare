...
struct StaticVector final {
    ...
    template <typename T>
    void fill(T &&t) {
        return m_Array.fill(std::forward<T>(t));
    }
    ...
    reference operator[](size_type _Pos) { return m_Array[_Pos]; }

    constexpr const_reference operator[](size_type _Pos) const { return m_Array[_Pos]; }
