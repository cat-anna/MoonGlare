...
template <typename T> constexpr bool HasPostInit = has_member_function_PostInit<void (T::*)()>::value;
} // namespace detail

class InterfaceMap : private boost::noncopyable {
public:
    template <typename T> void SetSharedInterface(std::shared_ptr<T> t) { AddInterface(t.get(), t); }

    template <typename T> void SetInterface(T *t) { AddInterface<T>(t, nullptr); }
    ...
    template <typename T> std::shared_ptr<T> GetSharedInterface() {
    ...
    template <typename T, typename... IFACES> void CreateObject() {
        auto shared_ptr = std::make_shared<T>(*this);
        SetSharedInterface(shared_ptr);
        (SetSharedInterface(std::static_pointer_cast<IFACES>(shared_ptr)), ...);
        objectActions.emplace_back(ObjectAction{shared_ptr, (void *)shared_ptr.get(), GetPostInitFunc<T>()});
    }
    ...
    template <typename T> void GetObject(T *&t) {
        t = GetInterface<T>();
        if (!t) {
            __debugbreak();
    ...
    // throws NotExistsException on error
    template <typename T> void GetObject(std::shared_ptr<T> &t) {
...
private:
    ...
    template <typename T> static void ExecutePostInit(void *p) { reinterpret_cast<T *>(p)->PostInit(); }

    template <typename T> static ActionFunc GetPostInitFunc() {
    ...
    template <typename T> void AddInterface(T *ptr, std::shared_ptr<T> shared_ptr) {
        ...
        if (shared_ptr)
            sharedInterfaces[std::type_index(typeid(T))] = shared_ptr;
        if (ptr)
