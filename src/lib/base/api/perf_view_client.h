...
struct PerfProducer {
    ...
    PerfProducer(InterfaceMap &ifaceMap) { ifaceMap.GetObject(perfClient); }

    ChartId AddChart(const std::string &name) { return perfClient->AddChart(name); }
    ...
    void AddData(SeriesId sid, float value) const { AddData(localSeriesId[sid], GetTimeAxisValue(), value); }

    Api::SingleData GetTimeAxisValue() const { return perfClient->GetTimeAxisValue(); }
...
private:
    ...
    void AddData(SeriesId sid, Api::SingleData x, Api::SingleData y) const { perfClient->AddData(sid, x, y); }
};
