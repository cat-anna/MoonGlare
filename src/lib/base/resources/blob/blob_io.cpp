...
bool BlobReader::ReadDataBlob(gsl::span<const uint8_t> BlobMemory, const DataHeader &header,
                              gsl::span<uint8_t> outputMemory) {
...
bool BlobReader::ReadDataBlob(gsl::span<const uint8_t> BlobMemory, const DataHeader &header,
    ...
    return ReadDataBlob(BlobMemory, header, {outputMemory.get(), header.dataSize});
}
