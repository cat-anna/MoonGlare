...
void SkeletalAnimationManager::ApplyAnimationData(SkeletalAnimationHandle handle, std::unique_ptr<uint8_t[]> memory,
...
void SkeletalAnimationManager::ResetBlendState(AnimationBlendState &state, const char *animSetName,
...
AnimationLoopState SkeletalAnimationManager::UpdateAnimation(AnimationBlendState &state, float dt,
                                                             BoneState *boneStates) {
    auto &as = state.state;
    auto *adata = GetAnimationData(as.handle);
    if (!adata)
