...
class SkeletalAnimationManager final {
  public:
    ...
    void ApplyAnimationData(SkeletalAnimationHandle handle,
                            std::unique_ptr<uint8_t[]> memory,
                            const SkeletalAnimation &animInfo);

    const SkeletalAnimation *GetAnimationData(SkeletalAnimationHandle handle);

    void ResetBlendState(AnimationBlendState &state, const char *animSetName,
                         BoneState *boneStates);

    AnimationLoopState UpdateAnimation(AnimationBlendState &state, float dt,
                                       BoneState *boneStates);
  ...
  private:
    ...
    using Bitmap =
        Memory::LinearAtomicBitmapAllocator<Configuration::AnimationLimit>;
    Bitmap allocationBitmap;

    iAsyncLoader *loader;

    std::unordered_map<std::string, SkeletalAnimationHandle>
        loadedAnimations; // temporary solution
