...
public:
    ...
    template <typename CMD> typename CMD::Argument *PushCommand(CommandKey SortKey = CommandKey()) {
    ...
    template <typename CMD, typename... ARGS> typename CMD::Argument *MakeCommandKey(CommandKey key, ARGS &&... args) {
        auto *argptr = AllocateMemory<CMD::Argument>(CMD::ArgumentSize());
        if (IsFull() || !argptr) {
            AddLogf(Warning, "Command queue is full. Command %s has not been allocated!", typeid(CMD).name());
            return nullptr;
    ...
    template <typename CMD, typename... ARGS> typename CMD::Argument *MakeCommand(ARGS &&... args) {
    ...
    bool PushQueue(CommandQueue *queue, CommandKey key = CommandKey()) {
        ...
        auto *argptr = AllocateMemory<ExecuteQueue::Argument>(static_cast<uint32_t>(ExecuteQueue::ArgumentSize()));
        if (IsFull() || !argptr) {
...
private:
    ...
    template <typename T> T *AllocateMemory(uint32_t size) {
