...
MeshManager::MeshManager(std::shared_ptr<iAsyncLoader> async_loader,
                         std::shared_ptr<iContextResourceLoader> context_loader)
    : async_loader(std::move(async_loader)), context_loader(std::move(context_loader)) {
...
MeshResourceHandle MeshManager::LoadMesh(FileResourceId res_id) {
    throw false;
}

MeshResourceHandle MeshManager::CreateMesh(const MeshSource &source, FileResourceId res_id) {
    throw false;
}
