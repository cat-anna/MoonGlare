...
static void l_print(lua_State *L) {
    ...
    if (n > 0) { /* any result to be printed? */
        ...
        if (lua_pcall(L, n, 0, 0) != LUA_OK)
            l_message("?", lua_pushfstring(L, "error calling 'print' (%s)", lua_tostring(L, -1)));
    }
}
...
bool CLI::InstallApi() {
    auto &sol = m_Lua->get_sol();

    auto classes = sol["classes"].get_or_create<sol::table>();

    classes.new_usertype<CLI>("Cli",             //
                              "Exit", &CLI::Exit //
    );
...
void CLI::Exit(int ec) { can_continue = false; }

bool CLI::Enter(InitEnv &env) {
    if (!m_Lua)
        return false;

    bash_mode = env.bash_mode;
