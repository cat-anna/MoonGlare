...
LuaDebugContextModule::LuaDebugContextModule(lua_State *lua,
                                             InterfaceMap *world)
    : interfaceMap(*world) {
    ...
    if (lua_isnil(lua, -1)) {
        ...
        lua_PushCClosure(lua, &LuaDebugContextModule::IndexMethod,
                         (void *)this);
...
ApiInitializer LuaDebugContextModule::RegisterScriptApi(ApiInitializer api) {
    return api.beginClass<CallableObject>("CallableObject")
        .addCFunction("__call", &CallableObject::Call)
        .endClass();
}

ApiInitializer
LuaDebugContextModule::RegisterDebugScriptApi(ApiInitializer api) {
    return api;
}
...
int LuaDebugContextModule::IndexMethod(lua_State *lua) {
    ...
    LuaDebugContextModule *ctx =
        reinterpret_cast<LuaDebugContextModule *>(This);

    auto debugCtx =
        ctx->interfaceMap.GetSharedInterface<Module::iDebugContext>();
