...
struct PerfViewClient::Internals {
    ...
    {
        socket.Send(data, MessageType::AddData);
    }
    void SendPacket(const Api::PayLoad_AddSeries& data)
    {
        socket.Send(data, MessageType::AddSeries);
    }
    void SendPacket(const Api::PayLoad_BeginSession& data)
    {
        socket.Send(data, MessageType::BeginSession);
    }
    void SendPacket(const Api::PayLoad_AddChart& data)
    {
        socket.Send(data, MessageType::AddChart);
    }
};
