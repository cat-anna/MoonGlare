...
struct FileSystemViewerInfo : public QtShared::DockWindowModule<FileSystemViewer>,
...
FileSystemViewer::FileSystemViewer(QWidget *parent, WeakModule module) : QtShared::DockWindow(parent) {

    m_EditorModule = module;
    auto mm = m_EditorModule.lock()->GetModuleManager();
    m_FileIconProvider = mm->QueryModule<QtShared::FileIconProvider>();
    m_EditorProvider = mm->QueryModule<QtShared::EditorProvider>();

    SetSettingID("FileSystemViewer");
    m_Ui = std::make_unique<Ui::FilesystemViewer>();
    m_Ui->setupUi(this);

    m_FileSystem = mm->QueryModule<FileSystem>();
...
bool FileSystemViewer::Create(const std::string &LocationURI, const QtShared::iEditorInfo::FileHandleMethodInfo &what) {
    QString qname;
    if (!QueryStringInput("Enter name:", qname))
        return false;
...
void FileSystemViewer::ShowContextMenu(const QPoint &point) {
    ...
    if (!fullpath.empty()) {
        if (folder) {
            ...
            for (auto methodmodule : methods) {
                ...
                CreateMenu->addAction(QIcon(method.m_Icon.c_str()), method.m_Caption.c_str(),
                                      [this, methodmodule, fullpath]() { OpenFileEditor(methodmodule, fullpath); });
            }
        } else {
            auto methods =
                m_EditorProvider.lock()->GetOpenMethods(boost::filesystem::path(fullpath).extension().string());
            if (methods.size() == 0) {
    ...
    menu.addAction("Collapse all", [this]() { m_Ui->treeView->collapseAll(); });
    menu.addAction("Expand all", [this]() { m_Ui->treeView->expandAll(); });
...
void FileSystemViewer::ItemDoubleClicked(const QModelIndex &) {
    ...
    try {
        auto editorAction =
            m_EditorProvider.lock()->FindOpenEditor(boost::filesystem::path(fullpath).extension().string());
...
void FileSystemViewer::OpenFileEditor(const MoonGlare::QtShared::EditorProvider::EditorActionInfo &editorAction,
...
void FileSystemViewer::ProjectChanged(Module::SharedDataModule datamod) { m_Module = datamod; }
