...
FileSystem::FileSystem(SharedModuleManager modmgr) : iModule(std::move(modmgr)), fsWatcher(this) {
    ...
    connect(&fsWatcher, &QFileSystemWatcher::directoryChanged,
            [this](const QString &path) { HandleDirectoryChanged(path.toLocal8Bit().begin()); });
