...
class World final : public InterfaceMap {
  public:
    ...
    Core::RuntimeConfiguration *GetRuntimeConfiguration() {
        return runtimeConfiguration.get();
    }

    Core::Scripts::ScriptEngine *GetScriptEngine() { return m_ScriptEngine; };
    Core::Scene::iSceneManager *GetScenesManager() {
        return m_ScenesManager.get();
    };
    ...
    void SetScriptEngine(Core::Scripts::ScriptEngine *se) {
        m_ScriptEngine = se;
    }
