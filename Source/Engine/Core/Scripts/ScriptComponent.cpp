...
ApiInitializer ScriptComponent::RegisterScriptApi(ApiInitializer root) { return GameObject::RegisterScriptApi(root); }
...
void ScriptComponent::Step(const SubsystemUpdateData &xconf) {
    ...
    for (size_t i = 1; i < m_Array.Allocated(); ++i) {
        ...
        if (!ScriptObject::Step(lua, movedataIndex, errf)) {
            ...
            AddLog(ScriptRuntime, "System : Warning : Missing function : Function 'Step' is not defined for entity: "
                                      << item.m_OwnerEntity);
        }
    }
    ...
    if (InvalidEntryCount > 0) {
        AddLogf(Performance, "ScriptComponent:%p InvalidEntryCount:%lu LastInvalidEntry:%lu", this, InvalidEntryCount,
                LastInvalidEntry);
...
bool ScriptComponent::Load(ComponentReader &reader, Entity parent, Entity owner) {
    ...
    if (se.m_Script[0] == '/') {
        if (!requireModule->Query(lua, se.m_Script)) {
    ...
    auto *gameObject = gameObjectTable->Set(owner, GetManager(), this, ifmap.GetInterface<EntityManager>(),
                                            ifmap.GetInterface<PrefabManager>());
    if (!gameObject) {
        // TODO
    }
    ...
    if (se.m_Properties.empty())
        ...
        if (loadStatus != 0) {
            ...
            AddLogf(Error, "Cannot parse script props: '%s' script: %s error: %s", se.m_Properties.c_str(),
                    se.m_Script.c_str(), errmesg);
            lua_pop(lua, 1);
            lua_createtable(lua, 0, 0);
        } else {
            ...
            if (succ != 0) {
                const char *errmesg = lua_tostring(lua, -1);
                AddLogf(Error, "Cannot execute script props: '%s' script: %s", se.m_Properties.c_str(),
                        se.m_Script.c_str(), errmesg);
                lua_pop(lua, 1);
                lua_createtable(lua, 0, 0);
            }
...
GameObject *ScriptComponent::GetGameObject(Entity Owner) {
    ...
    if (!ptr) {
        ...
        auto &ifmap = *GetManager()->GetWorld();
        ptr = gameObjectTable->Set(Owner, GetManager(), this, ifmap.GetInterface<EntityManager>(),
                                   ifmap.GetInterface<PrefabManager>());
        if (!ptr) {
            // TODO
        }
...
int ScriptComponent::lua_DereferenceHandle(lua_State *lua) {
    ...
    if (!cptr->PushEntryToLua(e, lua, rets)) {
        AddLogf(Error, "ComponentInstanceInfo::Get: Error: Component '%s' does not support lua api",
                typeid(*cptr).name());
        return 0;
    }
...
int ScriptComponent::lua_SetComponentState(lua_State *lua) {
    ...
    if (!cptr->PushEntryToLua(e, lua, rets)) {
        lua_pushvalue(lua, -2);
        AddLog(Error, "ComponentInstanceInfo::Set: Error: Component '" << typeid(*cptr).name()
                                                                       << "' does not support lua api owner:" << e);
        lua_settop(lua, argc);
        return 0;
    }
