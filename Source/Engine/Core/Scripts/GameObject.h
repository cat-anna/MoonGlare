...
struct GameObject {
    ...
    Component::SubsystemManager *myWorld;
    ...
    int GetComponent(lua_State * lua) {
        ...
        if (ssid == SubSystemId::Script) {
            return check.ReturnArgs(scriptComponent->lua_GetScriptComponent(lua, owner));
        }
...
struct GameObjectTable {
    ...
    GameObject* Set(Entity e, Component::SubsystemManager *world, ScriptComponent *scriptComponent, Component::EntityManager *entityManager) {
        auto &ptr = gameObjectTable[e.GetIndex()];
        ptr = {};
        ptr.owner = e;
