...
class ScenesManager : public iSceneManager {
  public:
    ...
    bool IsScenePending() const override {
        return pendingScene.load() != nullptr;
    }
  ...
  protected:
    ...
    std::atomic<SceneInstance *> nextScene =
        nullptr; // Scene to be activated at next loop
    std::atomic<SceneInstance *> pendingScene =
        nullptr; // Scene which is under initialization
    ...
    SceneDescriptor *CreateDescriptor(StarVFS::FileID fid,
                                      const std::string &name);
    SceneDescriptor *FindDescriptor(const std::string &name);

    SceneInstance *CreateScene(const std::string &descName,
