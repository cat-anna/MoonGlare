...
static void DefaultStarVFSLogSink(const char *file, const char *function, unsigned line, const char *log,
                                  const char *type) {}
void (*StarVFSLogSink)(const char *file, const char *function, unsigned line, const char *log,
                       const char *type) = DefaultStarVFSLogSink;
...
struct StarVFS::Internals {
    ...
    struct ContainerInfo {
        ...
        ContainerInfo(ContainerInfo &&oth)
            : m_Container(std::move(oth.m_Container)), m_MountPoint(std::move(oth.m_MountPoint)) {}
    };
...
StarVFS::StarVFS(unsigned FSFlags) : m_Callback(nullptr) {
...
void StarVFS::DumpStructure(std::ostream &out) const { m_FileTable->DumpStructure(out); }

void StarVFS::DumpFileTable(std::ostream &out) const { m_FileTable->DumpFileTable(out); }

void StarVFS::DumpHashTable(std::ostream &out) const { m_FileTable->DumpHashTable(out); }

//-----------------------------------------------------------------------------

bool StarVFS::IsFileValid(FileID fid) const { return m_FileTable->IsValid(fid); }
...
String StarVFS::GetFullFilePath(FileID fid) const { return m_FileTable->GetFilePath(fid, 0); }

String StarVFS::GetFilePath(FileID fid, FileID ParentFID) const { return m_FileTable->GetFilePath(fid, ParentFID); }

CString StarVFS::GetFileName(FileID fid) const { return m_FileTable->GetFileName(fid); }
...
bool StarVFS::GetFileData(FileID fid, ByteTable &data) { return m_FileTable->GetFileData(fid, data); }
...
bool StarVFS::DeleteFile(FileID fid) { return m_FileTable->DeleteFile(fid); }
...
HandleTable *StarVFS::GetHandleTable() { return m_Internals->m_HandleTable.get(); }

//-----------------------------------------------------------------------------

size_t StarVFS::GetModuleCount() const { return m_Internals->m_Modules.size(); }
