...
struct ThreadedSoundEngineModule : public MoonGlare::Modules::ModuleInfo, public Core::Scripts::Settings::iSettingsProvider {
    ...
    virtual bool Initialize() override {
        return (soundEngine = new SoundEngine(&settings))->Initialize();
    }
    ...
    bool LoadSettings(const pugi::xml_node node) override {
        using namespace MoonGlare::x2c::Modules::ThreadedSoundEngine;
        return SoundEngineSettings_Read(node, settings, nullptr);
    }
    bool SaveSettings(pugi::xml_node node) const override {
        using namespace MoonGlare::x2c::Modules::ThreadedSoundEngine;
    ...
    std::unordered_map<std::string, Setting> GetSettingList(std::string_view prefix) const override {
        ...
        return {
            ...
            { "Volume.Master", Setting{ ApplyMethod::Immediate } },
            { "Volume.Sound", Setting{ ApplyMethod::Immediate } },
    ...
    void Set(std::string_view prefix, std::string_view id, ValueVariant value) override {
        ...
        case "Enabled"_Hash32:
            ...
            soundEngine->ConfigurationChanged(SettingsGroup::Sound);
            return;
        case "Volume.Master"_Hash32:
            settings.volumeMaster = xclamp(std::get<float>(value));
            soundEngine->ConfigurationChanged(SettingsGroup::Sound_Volume);
            return;
        case "Volume.Sound"_Hash32:
            settings.volumeSound = xclamp(std::get<float>(value));
            soundEngine->ConfigurationChanged(SettingsGroup::Sound_Volume);
            return;
        case "Volume.Music"_Hash32:
            settings.volumeMusic = xclamp(std::get<float>(value));
            soundEngine->ConfigurationChanged(SettingsGroup::Sound_Volume);
            return;
        default:
    ...
    ValueVariant Get(std::string_view prefix, std::string_view id) override {
        ...
        case "Volume.Master"_Hash32:
            return settings.volumeMaster;
        case "Volume.Sound"_Hash32:
            return settings.volumeSound;
        case "Volume.Music"_Hash32:
            return settings.volumeMusic;
        default:
            throw InvalidSettingId{};
        };
    }
...
SoundEngine::SoundEngine(SoundEngineSettings *Settings) :
    BaseClass(),
...
struct SoundEngineDebugApi {
    ...
    static SoundEngine* se() {
        return static_cast<SoundEngine*>(GetSoundEngine());
    }
...
void SoundEngine::ConfigurationChanged(SettingsGroup what) {
    ...
    case SettingsGroup::Sound_Volume:
    {
        ...
        m_ActionList.push_back([this] {
            {
                LOCK_MUTEX(m_SoundsMutex);
                float v = settings->volumeSound * settings->volumeMaster;
                for (auto &it : m_Sounds)
            ...
            {
                LOCK_MUTEX(m_MusicMutex);
                float v = settings->volumeMusic * settings->volumeMaster;
                for (auto &it : m_Music)
                    if (it.second.m_Sound)
                        it.second.m_Sound->SetVolume(v);
            }
...
bool SoundEngine::SoundInfo::Load(SoundEngineSettings *settings) {
    ...
    if (m_IsMusic) {
        ...
        m_Sound->SetVolume(settings->volumeMaster * settings->volumeMusic);
    }
    else {
        m_Sound->SetVolume(settings->volumeMaster * settings->volumeSound);
    }
    return true;
}
