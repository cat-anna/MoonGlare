...
struct PerfServer::Internals : public UdpSocket<Api::MessageBuffer> {
    ...
    Internals(PerfServer *owner) : UdpSocket(), owner(owner) {
        Bind(Api::PerfViewPort);
        Listen();
    }

    void OnMessage(const MessageHeader *request,
                   const udp::endpoint &sender) override {
        ...
        case MessageType::BeginSession:
            return BeginSession(
                (const Api::PayLoad_BeginSession *)request->payLoad);
        default:
            __debugbreak();
            // todo
        }
...
PerfServer::PerfServer() { internals = std::make_unique<Internals>(this); }
...
void PerfServer::AddSeries(const std::string_view seriesName,
                           SeriesInfo seriesInfo, AxisInfo axisInfo) {}
