...
struct OutputBuffer : public std::list<std::string> {
    template <typename... ARGS> void pushf(const char *fmt, ARGS &&... args) {
        char buf[4 * 1024];
        sprintf_s(buf, fmt, std::forward<ARGS>(args)...);
        push_back(buf);
    }
    void pushs(const char *fmt, const std::string &str) { pushf(fmt, str.c_str()); }
    template <typename... ARGS> void push(const char *fmt, ARGS... args) {
...
private:
    ...
    using PreprocessorTokenHadler = void (Preprocessor::*)(const std::string &FName, const std::string &line, int level,
                                                           std::smatch match);
