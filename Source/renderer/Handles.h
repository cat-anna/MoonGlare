...
struct ShaderResourceHandleBase {
    ...
    operator bool() const { return m_TmpGuard == GuardValue; }
};
static_assert(std::is_pod<ShaderResourceHandleBase>::value, "Must be pod type!");

template <typename Descirptor> struct ShaderResourceHandle : public ShaderResourceHandleBase {};
...
template <typename DeviceHandle_t, typename DiffType = void> struct HandleTemplate : public ResourceHandleBase {
