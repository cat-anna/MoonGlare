...
#include <Memory/GenerationRandomAllocator.h>
...
TEST(Memory_GenerationRandomAllocator, test_0) {
	...
	using type_t = GenerationRandomAllocator<
...
//	std::cout << "Alloc handleD: " << PrintHandle(handleD, &allocator) << "\n";
	...
	ASSERT_EQ(4, allocator.GetRawIndex(handleD));
	ASSERT_EQ(3, allocator.Allocated());
...
//	std::cout << "handleE: " << PrintHandle(handleE, &allocator) << "\n";
	...
	ASSERT_EQ(5, handleE.GetIndex());
	ASSERT_EQ(5, allocator.GetRawIndex(handleE));
...
TEST(Memory_GenerationRandomAllocator, random) {
...
TEST(Memory_GenerationRandomAllocator, generation_overflow_attempt) {
	using Handle_t = DoubleHandle32<8>;
