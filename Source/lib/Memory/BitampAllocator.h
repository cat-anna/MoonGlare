...
template<size_t SIZE, typename ITEMTYPE, typename INDEXTYPE>
struct BitmapAllocatorBase {
	...
	static constexpr Index_t MakeIndex(Index_t slot, Index_t bit) {
		return slot * BitsPerSlot + bit;
	}
	static constexpr SlotAndBit SplitIndex(Index_t index) {
		return SlotAndBit{ index / BitsPerSlot, index & (BitsPerSlot - 1) };
	}
	static constexpr Item_t MakeMask(Index_t bit) {
		return static_cast<Item_t>(1u) << bit;
	}
};
